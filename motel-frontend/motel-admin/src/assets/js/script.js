...
function checkEmailUnique(form, url) {
    email = $("#email").val();
    accId = $("#id").val();
    csrfValue = $("input[name='_csrf']").val();
    params = {id: accId, email: email, _csrf: csrfValue};

    $.post(url, params, function (response) {
        if (response == "OK") {
            form.submit();
        } else if (response == "Duplicated") {
            showModalDialog("Cảnh báo", "Đã có khách hàng khác sử dụng email: " + email);
        } else {
            showModalDialog("Lỗi", "Unknown response from server");
        }
    }).fail(function () {
        showModalDialog("Lỗi", "Không thể kết nối tới server")
    })
    return false;
}

//checkemail Unique
function checkUsernameUnique(form, url) {

    username = $("#username").val();
    accId = $("#id").val();
    csrfValue = $("input[name='_csrf']").val();
    params = {id: accId, username: username, _csrf: csrfValue};

    $.post(url, params, function (response) {
        if (response == "OK") {
            form.submit();
        } else if (response == "Duplicated") {
            showModalDialog("Cảnh báo", "Đã có tài khoản khác sử dụng username: " + username);
        } else {
            showModalDialog("Lỗi", "Unknown response from server");
        }
    }).fail(function () {
